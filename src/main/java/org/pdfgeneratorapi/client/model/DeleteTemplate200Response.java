...
import org.pdfgeneratorapi.client.model.DeleteTemplate200ResponseResponse;
...
public class DeleteTemplate200Response {
  ...
  * @throws IOException if the JSON Object is invalid with respect to DeleteTemplate200Response
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!DeleteTemplate200Response.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DeleteTemplate200Response.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DeleteTemplate200Response` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      // validate the optional field `response`
      if (jsonObj.get("response") != null && !jsonObj.get("response").isJsonNull()) {
        DeleteTemplate200ResponseResponse.validateJsonObject(jsonObj.getAsJsonObject("response"));
      }
  }
