...
public class CreateTemplate200Response {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CreateTemplate200Response
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CreateTemplate200Response.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CreateTemplate200Response.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreateTemplate200Response` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
