...
public class TemplateDefinitionNewLayout {
  ...
  **/
  ...
  public TemplateDefinitionNewLayout rotation(RotationEnum rotation) {
  ...
  * @throws IOException if the JSON Object is invalid with respect to TemplateDefinitionNewLayout
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!TemplateDefinitionNewLayout.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!TemplateDefinitionNewLayout.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TemplateDefinitionNewLayout` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
