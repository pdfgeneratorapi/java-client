...
public class GetTemplates429Response {
  ...
  public GetTemplates429Response error(@javax.annotation.Nullable ErrorEnum error) {
    this.error = error;
    return this;
  }
  ...
  public GetTemplates429Response status(@javax.annotation.Nullable Integer status) {
    this.status = status;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to GetTemplates429Response
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!GetTemplates429Response.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!GetTemplates429Response.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `GetTemplates429Response` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
