...
public class DeleteTemplate200ResponseResponse {
  ...
  * @throws IOException if the JSON Object is invalid with respect to DeleteTemplate200ResponseResponse
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!DeleteTemplate200ResponseResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!DeleteTemplate200ResponseResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DeleteTemplate200ResponseResponse` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  }
