...
public class OpenEditorRequestData extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(OpenEditorRequestData.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!OpenEditorRequestData.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'OpenEditorRequestData' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<OpenEditorRequestData>() {
                @Override
                public void write(JsonWriter out, OpenEditorRequestData value) throws IOException {
                ...
                @Override
                public OpenEditorRequestData read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        OpenEditorRequestData ret = new OpenEditorRequestData();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for OpenEditorRequestData: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public OpenEditorRequestData() {
        super("oneOf", Boolean.FALSE);
    }

    public OpenEditorRequestData(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return OpenEditorRequestData.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to OpenEditorRequestData
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for OpenEditorRequestData with oneOf schemas: List<Object>, Object. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of OpenEditorRequestData given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of OpenEditorRequestData
     * @throws IOException if the JSON string is invalid with respect to OpenEditorRequestData
     */
    public static OpenEditorRequestData fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, OpenEditorRequestData.class);
    }

    /**
     * Convert an instance of OpenEditorRequestData to an JSON string
     *
     * @return JSON string
     */
