...
public class TemplateParamId extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(TemplateParamId.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!TemplateParamId.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'TemplateParamId' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<TemplateParamId>() {
                @Override
                public void write(JsonWriter out, TemplateParamId value) throws IOException {
                ...
                @Override
                public TemplateParamId read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        TemplateParamId ret = new TemplateParamId();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for TemplateParamId: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public TemplateParamId() {
        super("oneOf", Boolean.FALSE);
    }

    public TemplateParamId(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return TemplateParamId.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to TemplateParamId
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for TemplateParamId with oneOf schemas: Integer, String. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of TemplateParamId given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of TemplateParamId
     * @throws IOException if the JSON string is invalid with respect to TemplateParamId
     */
    public static TemplateParamId fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, TemplateParamId.class);
    }

    /**
     * Convert an instance of TemplateParamId to an JSON string
     *
     * @return JSON string
     */
